...
// default vectra capacity is 64 bytes.
...
/*
 * vectra is a vector-like container that uses a variadic size buffer to store elements.
 * it is designed to be used in the non-arena memory.
 */
template <typename T, typename Alloc = std::allocator<T>>
class vectra : public core<T>
{
   public:
    using size_type = std::size_t;
    using value_type = T;
     ...
     */
    constexpr vectra() : core<T>() {}

    constexpr explicit vectra([[maybe_unused]] const Alloc& alloc) : core<T>() {}

    /*
     * constructor with capacity
     */
    constexpr explicit vectra(std::size_t size) : core<T>(size, size) {
        if (size > 0) {
            construct_range(this->_start, this->_finish);
        }
    }

    constexpr vectra(std::size_t size, const T& value) : core<T>(size, size) {
        if (size > 0) {
    ...
    constexpr vectra(InputIt first, InputIt last) : core<T>() {
        int64_t size = last - first;
        // if size == 0, then do nothing.and just for caller convenience.
        Assert(size >= 0, "vectra should be constructed with non-negative size");
        Assert((size_type)size <= this->max_size(), "vectra size should be smaller than max_size");
        if (size > 0) [[likely]] {
    ...
    constexpr vectra(std::initializer_list<T> init) : vectra(init.begin(), init.end()) {}

    /*
     * copy constructor of vectra
     */
    constexpr vectra(const vectra& other) = default;

    /*
     * copy assignment operator of vectra
     */
    constexpr auto operator=(const vectra& other) -> vectra& = default;

    /*
     * move constructor of vectra
     */
    constexpr vectra(vectra&&) noexcept = default;

    /*
     * move assignment operator of vectra
     */
    auto operator=(vectra&&) noexcept -> vectra& = default;
    ...
    constexpr void assign(Iterator first, Iterator last) {
        ...
        Assert(size_to_assign >= 0, "vectra should be assigned with non-negative size");
    ...
    [[gnu::always_inline]] constexpr inline void swap(vectra& other) noexcept { core<T>::swap(other); }
...
template <typename T>
auto operator==(const vectra<T>& lhs, const vectra<T>& rhs) -> bool {
...
template <typename T>
auto operator<=>(const vectra<T>& lhs, const vectra<T>& rhs) -> std::strong_ordering {
...
template <typename T>
constexpr void swap(stdb::container::vectra<T>& lhs, stdb::container::vectra<T>& rhs) {
    lhs.swap(rhs);
}

template <class T, class U>
constexpr auto erase(stdb::container::vectra<T>& vec, const U& value) -> std::size_t {
